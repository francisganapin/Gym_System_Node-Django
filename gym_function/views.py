...
def loginMember_views(request):
    if request.method == 'POST':
        try:
            ...
            # select expiry
            cursor.execute('SELECT expiry,first_name,last_name FROM gym_members WHERE id_card=%s', [id_card])
            result = cursor.fetchone()

            # select data  result
            expiry_result = result[0] if result else 'N/A'
            first_result  = result[1] if result else 'N/A'
            last_result   = result[2] if result else 'N/A'

            return JsonResponse({'success': True, 'message': f'Member expiry was {expiry_result}, {first_result}, {last_result}'})
